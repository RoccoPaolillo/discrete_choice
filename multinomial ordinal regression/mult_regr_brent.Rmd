---
title: "Multinomial Logistic Regression Brent"
output: pdf_document
---
...
wd <- setwd("C:/Users/rocpa/OneDrive/Desktop/brent3")
...
data_cat <- data %>% select(GEO_LABEL,white, black,asian,diveth,high, medium,low,divses)
...
# 
 ...
 keep(dataset,data_cat, sure = TRUE)
 
 # plot to check aggregated number consistency
 plot_pop <- ggplot(dataset,aes(dataset$ses,fill=dataset$ses)) + geom_bar() + facet_wrap(dataset$ethnic) + scale_fill_manual( values =c(high = "green",medium = "orange",low ="blue")) + geom_text(stat="count", aes(label=..count..),vjust = 0) 
 plot_pop

# ggsave("population.png",plot_pop)
